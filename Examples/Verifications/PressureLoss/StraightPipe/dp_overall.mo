...
model dp_overall "Verification of function dp_overall"
  ...
  Modelica.Units.SI.Diameter d_hyd=0.1 "Hydraulic diameter"
    annotation (Dialog(group="Straight pipe"));
  Modelica.Units.SI.Length K[4]={2e-5*d_hyd,2e-4*d_hyd,2e-3*d_hyd,2e-2*d_hyd}
  ...
  //  //plotting
  Modelica.Units.SI.Velocity velocity[n]={abs(input_mdot[i])/(rho*A_cross) for
      i in 1:n} "Mean velocity";
  Modelica.Units.SI.ReynoldsNumber Re[n]={rho*abs(velocity[i])*d_hyd/eta for i
       in 1:n};
