...
model dp_internalFlowOverall
  ...
  Modelica.Units.SI.MassFlowRate input_mdot_1[n](start=zeros(n)) = {
    input_mflow_0.y*(d_hydT[2]/d_hydT[i])*(A_crossT[i]/A_crossT[2]) for i in 1:
    n} "(Input) mass flow rate (for intended incompressible case)";
...
protected
  ...
  Modelica.Units.SI.ReynoldsNumber Re_1[n]={rho*velocity_1[i]*d_hydT[i]/eta
      for i in 1:n};
