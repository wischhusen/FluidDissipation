...
package RoundTubeBundle "FluidDissipation: Heat Exchanger with tube bundles"
  ...
  model RoundTubeBundleHeatTransferModel
    ...
    parameter Modelica.Units.SI.Length d(min=1e-6) = 0.02
      "Outer diameter of tubes" annotation (Dialog(group="HeatExchanger"));
    parameter Modelica.Units.SI.Length s_1(min=2*1e-6) = 0.03
      "Distance between tubes (center to center) orthogonal to flow direction"
      annotation (Dialog(group="HeatExchanger"));
    parameter Modelica.Units.SI.Length s_2(min=0) = 0.026
      "Distance between tubes (center to center) parallel to flow direction"
      annotation (Dialog(group="HeatExchanger"));
    parameter Modelica.Units.SI.Length L_pipe(min=1e-6) = 1
      "Distance between tubes (center to center) parallel to flow direction"
      annotation (Dialog(group="HeatExchanger"));
  ...
  annotation (Dialog(group="HeatExchanger"));
    ...
    Modelica.Units.SI.Area A_cross=A_front "Cross-sectional area";
    Modelica.Units.SI.Area A_kc=2*L_Re*L_pipe*z*n
    ...
    Modelica.Units.SI.Velocity velocity=abs(m_flow)/max(Modelica.Constants.eps,
        (IN_var.rho*A_cross)) "Mean velocity in free flow area";
    Modelica.Units.SI.Length L_Re=0.5*Modelica.Constants.pi*d
    ...
    Modelica.Units.SI.ReynoldsNumber Re_psi=IN_var.rho*velocity*L_Re/psi/IN_var.eta
      "Reynolds number for the void cross-sectional area";
    Modelica.Units.SI.NusseltNumber Nu=kc*L_Re/IN_var.lambda "Nusselt number";
