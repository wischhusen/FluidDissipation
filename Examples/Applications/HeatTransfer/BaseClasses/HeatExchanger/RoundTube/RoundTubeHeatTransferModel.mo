...
model RoundTubeHeatTransferModel
  ...
  parameter Modelica.Units.SI.Length F_p=0.002
    "Fin pitch, fin spacing + fin thickness" annotation (Dialog(group=
          "HeatExchanger", enable=if geometry == FluidDissipation.Utilities.Types.HTXGeometry_roundTubes.PlainFin
           ...
           or geometry == FluidDissipation.Utilities.Types.HTXGeometry_roundTubes.WavyFin
  ...
  parameter Modelica.Units.SI.Length P_t=0.025 "Transverse tube pitch"
    annotation (Dialog(group="HeatExchanger"));
  parameter Modelica.Units.SI.Length S_h=0.001 "Slit height" annotation (Dialog(
        group="HeatExchanger", enable=if geometry == FluidDissipation.Utilities.Types.HTXGeometry_roundTubes.SlitFin
           then true else false));
  parameter Modelica.Units.SI.Length S_p=0.002 "Slit pitch" annotation (Dialog(
        group="HeatExchanger", enable=if geometry == FluidDissipation.Utilities.Types.HTXGeometry_roundTubes.SlitFin
           then true else false));
  ...
  parameter Modelica.Units.SI.Length delta_f=0.0001 "Fin thickness" annotation
    (Dialog(group="HeatExchanger", enable=if geometry == FluidDissipation.Utilities.Types.HTXGeometry_roundTubes.PlainFin
           or geometry == FluidDissipation.Utilities.Types.HTXGeometry_roundTubes.LouverFin
           or geometry == FluidDissipation.Utilities.Types.HTXGeometry_roundTubes.SlitFin
           or geometry == FluidDissipation.Utilities.Types.HTXGeometry_roundTubes.WavyFin
           then true else false));
  ...
  Modelica.Units.SI.Area A_cross=A_fr*((F_p*P_t - F_p*D_c - (P_t - D_c)*delta_f)
      /(F_p*P_t)) "Minimum flow cross-sectional area";
  Modelica.Units.SI.Length D_h=if geometry == FluidDissipation.Utilities.Types.HTXGeometry_roundTubes.LouverFin
       then 4*A_cross/(A_fr*(2*(P_t - D_c + F_p)/(F_p*(P_t - D_c)))) else 0
    "Hydraulic diameter";

  Modelica.Units.SI.Area A_kc=if geometry == FluidDissipation.Utilities.Types.HTXGeometry_roundTubes.PlainFin
       or geometry == FluidDissipation.Utilities.Types.HTXGeometry_roundTubes.LouverFin
       or geometry == FluidDissipation.Utilities.Types.HTXGeometry_roundTubes.SlitFin
       then A_fr*((N*PI*D_c*(F_p - delta_f) + 2*(P_t*L - N*PI*D_c^2/4))/(P_t*
      F_p)) else if geometry == FluidDissipation.Utilities.Types.HTXGeometry_roundTubes.WavyFin
       then A_fr*((N*PI*D_c*(F_p - delta_f) + 2*(P_t*L - N*PI*D_c^2/4)*(sqrt(
  ...
  Modelica.Units.SI.ReynoldsNumber Re=rho*velocity*D_c/eta
    "Reynolds number based on fin collar diameter";
  Modelica.Units.SI.NusseltNumber Nu=kc*D_c/lambda "Nusselt number";
