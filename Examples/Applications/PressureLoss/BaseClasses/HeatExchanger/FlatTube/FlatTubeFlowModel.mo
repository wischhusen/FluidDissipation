...
model FlatTubeFlowModel
  ...
  parameter Modelica.Units.SI.Length D_m=0.005
    "Major tube diameter for flat tube"
    annotation (Dialog(group="HeatExchanger"));
  parameter Modelica.Units.SI.Length F_d=0.02 "Fin depth" annotation (Dialog(
        group="HeatExchanger", enable=if geometry == FluidDissipation.Utilities.Types.HTXGeometry_flatTubes.LouverFin
           then true else false));
  parameter Modelica.Units.SI.Length F_l=0.02 "Fin length" annotation (Dialog(
        group="HeatExchanger", enable=if geometry == FluidDissipation.Utilities.Types.HTXGeometry_flatTubes.LouverFin
           then true else false));
  ...
  parameter Modelica.Units.SI.Length L=0.1 "Heat exchanger length"
    annotation (Dialog(group="HeatExchanger"));
  parameter Modelica.Units.SI.Length L_l=0.0015 "Louver length" annotation (
      Dialog(group="HeatExchanger", enable=if geometry == FluidDissipation.Utilities.Types.HTXGeometry_flatTubes.LouverFin
           then true else false));
  parameter Modelica.Units.SI.Length L_p=0.0015 "Louver pitch" annotation (
      Dialog(group="HeatExchanger", enable=if geometry == FluidDissipation.Utilities.Types.HTXGeometry_flatTubes.LouverFin
           then true else false));
  parameter Modelica.Units.SI.Length T_p=0.02 "Tube pitch" annotation (Dialog(
        group="HeatExchanger", enable=if geometry == FluidDissipation.Utilities.Types.HTXGeometry_flatTubes.LouverFin
           then true else false));
